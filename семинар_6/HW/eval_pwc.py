...
CASES = [
    ...
    
    {
        "id": "Q4",
        "query": (
            "Сравни курс рубля к доллару, евро и юаню на сегодня. "
            "Какая валюта укрепилась сильнее всего с начала 2025 года?"
        ),
        "comment": (
            "ГАРАНТИРОВАННО ЧИНИТСЯ ВАЛИДАТОРОМ: требует 3 вызова get_fx_rate "
            "(для USD, EUR, CNY) + calculate для сравнения. Без валидатора агент "
            "может вызвать несуществующий инструмент get_all_fx_rates или "
            "get_currency_strength. Валидатор отловит галлюцинацию."
        ),
        "expected_tools_pwc": {"get_fx_rate", "calculate"},
        "must_have_keywords": ["доллар", "евро", "юань", "укрепилась"],
        "forbid_hallucinated_tools": True,
    },
    {
        "id": "Q5",
        "query": (
            "Сравни инфляцию за 2022, 2023, 2024, 2025 годы. "
            "В каком году инфляция была максимальной?"
        ),
        "comment": (
            "ЕСТЕСТВЕННАЯ ПАРАЛЛЕЛЬНОСТЬ: 4 независимых вызова get_inflation "
            "(по годам) + calculate для сравнения. Подвопросы не зависят друг от друга "
            "→ можно выполнять параллельно для ускорения."
        ),
        "expected_tools_pwc": {"get_inflation", "calculate"},
        "must_have_keywords": ["инфляция", "максимальной", "год"],
        "forbid_hallucinated_tools": True,
    },
    {
        "id": "Q6",
        "query": (
            "Если я вложу 1 млн рублей в облигации с доходностью, равной "
            "текущей ключевой ставке, и инфляция останется на уровне прошлого года, "
            "какая будет реальная доходность через год? Какую сумму я получу?"
        ),
        "comment": (
            "РЕАЛЬНЫЙ МАКРО-ВОПРОС: классическая инвестиционная задача. "
            "Требует get_key_rate, get_inflation и calculate. Интересно для "
            "личных финансов и понимания реальной стоимости денег."
        ),
        "expected_tools_pwc": {"get_key_rate", "get_inflation", "calculate"},
        "must_have_keywords": ["реальная", "доходность", "рублей", "получу"],
        "forbid_hallucinated_tools": True,
    },
]
...
def run_case(case: dict, *, n: int = 5, validate: bool = False) -> dict:
    single = {"runs": [], "pass": 0}
    pwc = {"runs": [], "pass": 0}
    ...
    for i in range(n):
        ...
        # --- PWC ---
        try:
            r2 = run_pwc(case["query"], max_iter=3, verbose=False, validate=validate)
        except Exception as e:
