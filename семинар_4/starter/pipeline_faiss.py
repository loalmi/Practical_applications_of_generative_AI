"""
pipeline_faiss.py — RAG с FAISS (dense) + BM25 (sparse) + RRF
"""
...
def ingest():
    """Индексация: создаём FAISS-индекс и BM25-кэш"""
    all_chunks = []
    all_ids = []
...
def load_faiss():
    """Загружает FAISS индекс и чанки"""
...
def hybrid_retrieve(query: str, k: int = 5, top: int = 15, c: int = 60):
    """Гибридный поиск: Dense (FAISS) + Sparse (BM25) + RRF"""
...
def build_prompt(query: str, hits: dict) -> str:
    ...
    return (
        ...
        "2. Если есть ответ — приведи 1-5 точных цитат (НЕ пересказ), с именами.\n"
        "3. В `sources` — id блоков, откуда взяты цитаты.\n"
        "4. В `confidence` — 0.9+ если прямой ответ, 0.5-0.8 если собран из кусков, <0.5 если нет ответа.\n\n"
        f"Контекст:\n{ctx}\n\n"
        f"Вопрос: {query}\n\n"
...
def ask(query: str):
    print(f"Поиск (dense + sparse + RRF): {query}")
    t0 = time.time()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Использование: python pipeline_faiss.py {ingest|ask} [вопрос]")
        sys.exit(1)
    ...
    if cmd == "ingest":
        ...
        if len(sys.argv) < 3:
            print('Нужен вопрос: python pipeline_faiss.py ask "..."')
            sys.exit(1)
        ask(sys.argv[2])
    else:
