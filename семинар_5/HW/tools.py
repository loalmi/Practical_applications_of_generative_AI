...
# ===========================================================================
# 5. Сравнение метрик за два периода
# ===========================================================================
...
def compare_periods(
    ...
    Returns:
        {"metric": ..., "a": {"date": ..., "value": ...},
         ...
         "source": "..."}
    """
    # Определяем тип периода и парсим
    def parse_period(p: str):
        if len(p) == 7 and p[4] == '-':  # YYYY-MM
            year, month = map(int, p.split('-'))
            return {"type": "month", "year": year, "month": month}
        else:  # YYYY-MM-DD
            return {"type": "date", "date": p}

    def get_value(m: str, p: dict):
        if m.startswith("fx_"):
            currency = m[3:]  # USD, EUR, CNY
            if p["type"] == "date":
                return get_fx_rate(currency, on_date=p["date"])
            else:
                # Для месяца берём середину месяца
                date_str = f"{p['year']}-{p['month']:02d}-15"
                return get_fx_rate(currency, on_date=date_str)
        elif m == "key_rate":
            if p["type"] == "date":
                return get_key_rate(on_date=p["date"])
            else:
                date_str = f"{p['year']}-{p['month']:02d}-15"
                return get_key_rate(on_date=date_str)
        elif m == "cpi":
            if p["type"] == "date":
                # Для CPI нужен месяц, берём из даты
                d = datetime.strptime(p["date"], "%Y-%m-%d")
                return get_inflation(year=d.year, month=d.month)
            else:
                return get_inflation(year=p["year"], month=p["month"])
        elif m == "unemployment":
            if p["type"] == "date":
                d = datetime.strptime(p["date"], "%Y-%m-%d")
                return get_unemployment(year=d.year, month=d.month)
            else:
                return get_unemployment(year=p["year"], month=p["month"])
        else:
            return {"error": f"неизвестная метрика: {m}"}

    # Получаем значения
    a_p = parse_period(period_a)
    b_p = parse_period(period_b)

    a_res = get_value(metric, a_p)
    b_res = get_value(metric, b_p)

    # Проверяем ошибки
    if "error" in a_res:
        return {"error": f"ошибка получения значения для {period_a}: {a_res['error']}"}
    if "error" in b_res:
        return {"error": f"ошибка получения значения для {period_b}: {b_res['error']}"}

    # Извлекаем числовое значение
    def extract_val(res: dict):
        if metric.startswith("fx_"):
            return res.get("rate")
        elif metric == "key_rate":
            return res.get("rate")
        elif metric == "cpi":
            return res.get("cpi_yoy")
        elif metric == "unemployment":
            return res.get("unemployment")
        return None

    a_val = extract_val(a_res)
    b_val = extract_val(b_res)

    if a_val is None or b_val is None:
        return {"error": f"не удалось извлечь значение из ответа"}

    return {
        "metric": metric,
        "a": {"period": period_a, "value": a_val},
        "b": {"period": period_b, "value": b_val},
        "delta": round(b_val - a_val, 4),
        "ratio": round(b_val / a_val, 4) if a_val != 0 else None,
        "source": f"{a_res.get('source', 'unknown')} + {b_res.get('source', 'unknown')}"
    }